...
function HighlightCard({ item }: { item: WorkHighlight }) {
  ...
  return (
    <div className="card-interactive p-6 flex flex-col gap-3">
      ...
      <h3 className="text-sm font-semibold text-[#F4F4F4]">{item.title}</h3>
            ...
            <div className="space-y-0">
              {timeline.map((entry, i) => (
                <motion.article
                  key={i}
                  variants={fadeUp}
                  className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-6 lg:gap-16 py-10 border-b border-[#334155] last:border-0"
                >
                  {/* ── Meta column (left) ── */}
                  <div className="shrink-0 space-y-3">
                    ...
                    <p className="text-xs font-mono text-[#94A3B8] font-medium mt-1">
                      {entry.company}
                    </p>
                  </div>
                  ...
                  <div className="space-y-8">
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <h3 className="text-lg font-semibold text-[#F4F4F4]">{entry.role}</h3>
                      </div>
                      ...
                      <div>
                        ...
                        <ul className="space-y-2">
                          {entry.responsibilities.map((item, j) => (
                            <li key={j} className="flex items-start gap-2.5 text-sm text-[#94A3B8]">
                              <span className="w-1 h-1 rounded-full bg-[#64748B] shrink-0 mt-2" />
                              {item}
                            </li>
                          ))}
                        </ul>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {entry.technologies.map((tech) => (
                            <span
                              key={tech}
                              className="text-xs font-mono text-[#94A3B8] border border-[#334155] bg-[#1E293B] px-2.5 py-1 rounded hover:text-[#F4F4F4] hover:border-[#475569] transition-colors cursor-default"
                            >
                              {tech}
                            </span>
                          ))}
        ...
        <div className="max-w-6xl mx-auto px-6 md:px-12">
          ...
          >
            <motion.div variants={fadeUp} className="flex items-center gap-3 mb-4">
              ...
              <h2 className="font-display text-3xl md:text-4xl font-bold text-[#F4F4F4]">
