...
try {
    $login = Invoke-RestMethod -Uri "$BASE/auth/login" `
        ...
        -WebSession $session `
        -ErrorAction Stop
    Write-Host "  Logado como: $($login.user.email)" -ForegroundColor Green
} catch {
...
# ── 3. Inserir projetos ──────────────────────────────────────────────────────
Write-Host "`nInserindo projetos..." -ForegroundColor Cyan
$ok = 0; $skip = 0; $fail = 0

foreach ($p in $projects) {
    try {
        $body = ConvertTo-Json $p -Depth 3
        $result = Invoke-RestMethod -Uri "$BASE/projects" `
            -Method POST `
            -ContentType "application/json; charset=utf-8" `
            -Body ([System.Text.Encoding]::UTF8.GetBytes($body)) `
            -WebSession $session `
            -ErrorAction Stop
        Write-Host "  ✓  $($p.slug)" -ForegroundColor Green
        $ok++
    } catch {
        $status = $_.Exception.Response.StatusCode.value__
        if ($status -eq 409) {
            Write-Host "  ↩  $($p.slug) — ja existe" -ForegroundColor Yellow
            $skip++
        } else {
            Write-Host "  ✗  $($p.slug) — erro $status : $_" -ForegroundColor Red
            $fail++
        }
    }
}

Write-Host "`nConcluido: $ok inseridos · $skip ja existiam · $fail erros" -ForegroundColor Cyan
